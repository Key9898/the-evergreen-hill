...
const OurStoryImg = '/OurStory/our_story_banner_img.jpg'

interface OurStoryBannerProps {
  onNavigate?: (page: string) => void
}

export default function OurStoryBanner({ onNavigate }: OurStoryBannerProps) {
  const { fadeInDown, fadeInUp, staggerContainer } = useAnimation()
  const breadcrumbPages = [{ name: 'Our Story', href: '#ourStory', current: true }]
      ...
      <div className="relative mx-auto flex max-w-4xl flex-col items-center px-4 sm:px-6 pt-56 pb-20 sm:pt-64 sm:pb-32 lg:pt-72 lg:pb-40 text-center lg:px-8">
        ...
        >
          ...
          <motion.p
            ...
            variants={fadeInUp}
          >
            Every great sanctuary begins with a story. Discover the passion, heritage, and vision
            that brought The Evergreen Hill to life in the serene landscape of Kalaw.
          </motion.p>
        </motion.div>
      </div>
    </div>
